...
- (void)commonSetup {
	...
	[self.inputTextField addTarget:self
	 action:@selector(inputTextFieldEditingDidBegin)
	 forControlEvents:UIControlEventEditingDidBegin];
	[self.inputTextField addTarget:self
	 action:@selector(inputTextFieldEditingDidEnd)
...
- (void)layoutSubviews {
	...
	for (UIButton *button in self.mutableTagButtons) {
		...
		if (CGRectGetMaxX(previousButtonFrame) + self.interitemSpacing +
			...
			buttonFrame.origin.y =
			        MAX(CGRectGetMaxY(lowerFrame), CGRectGetMaxY(previousButtonFrame));
	...
	if (self.editable) {
		...
		if (self.mutableTagButtons.count == 0) {
		...
		} else if (CGRectGetMaxX(previousButtonFrame) + self.interitemSpacing +
		           textfieldFrame.size.width <=
		           contentWidth) {
			textfieldFrame.origin.x =
			        self.interitemSpacing + CGRectGetMaxX(previousButtonFrame);
			...
			case RKTagsViewTextFieldAlignCenter:
				textfieldFrame.origin.y =
				        CGRectGetMinY(previousButtonFrame) +
				        (previousButtonFrame.size.height - textfieldFrame.size.height) / 2;
				break;
			case RKTagsViewTextFieldAlignBottom:
				textfieldFrame.origin.y =
				        CGRectGetMinY(previousButtonFrame) +
				        (previousButtonFrame.size.height - textfieldFrame.size.height);
			}
			...
			case RKTagsViewTextFieldAlignTop:
				textfieldFrame.origin.y =
				        CGRectGetMaxY(previousButtonFrame) + self.lineSpacing;
				break;
			case RKTagsViewTextFieldAlignCenter:
				textfieldFrame.origin.y =
				        CGRectGetMaxY(previousButtonFrame) + self.lineSpacing +
			...
			CGRect nextButtonFrame =
			        CGRectMake(0, CGRectGetMaxY(previousButtonFrame) + self.lineSpacing,
			                   0, previousButtonFrame.size.height);
			lowerFrame =
			        (CGRectGetMaxY(textfieldFrame) < CGRectGetMaxY(nextButtonFrame))
			        ? nextButtonFrame
				: textfieldFrame;
		}
	...
	if (oldContentSize.height != self.contentSize.height) {
		...
		if ([self.delegate
		     respondsToSelector:@selector(tagsViewContentSizeDidChange:)]) {
			[self.delegate tagsViewContentSizeDidChange:self];
		}
	}
	// layout becomeFirstResponder button
	self.becomeFirstResponderButton.frame = CGRectMake(
	        -self.scrollView.contentInset.left, -self.scrollView.contentInset.top,
...
- (void)setEditable:(BOOL)editable {
	...
	if (editable) {
		...
		self.becomeFirstResponderButton.hidden =
...
- (BOOL)addTag:(NSString *)tag {
	return [self insertTag:tag atIndex:self.mutableTags.count];
}
...
- (BOOL)insertTag:(NSString *)tag atIndex:(NSInteger)index {
	...
	if (index >= 0 && index <= self.mutableTags.count) {
		...
		UIButton *tagButton;
		if ([self.delegate
		     respondsToSelector:@selector(tagsView:buttonForTagAtIndex:)]) {
...
- (void)moveTagAtIndex:(NSInteger)index toIndex:(NSInteger)newIndex {
	if (index >= 0 && index <= self.mutableTags.count && newIndex >= 0 &&
	    newIndex <= self.mutableTags.count && index != newIndex) {
...
- (void)inputTextFieldChanged {
	...
	NSMutableArray *tags = [[(self.inputTextField.text ? : @"")
	                         componentsSeparatedByCharactersInSet:self.deliminater] mutableCopy];
	self.inputTextField.text = @""; //[tags lastObject];
	                                //  [tags removeLastObject];
	for (NSString *tag in tags) {
		if ([tag isEqualToString:@""] ||
		    ([self.delegate
		      respondsToSelector:@selector(tagsView:shouldAddTagWithText:)] &&
		     ![self.delegate tagsView:self shouldAddTagWithText:tag])) {
			continue;
		}
        if(![self addTag:tag] && tag.length>0){
            self.inputTextField.text =tag;
        }
	...
	dispatch_after(
		...
		if (self.scrollView.contentSize.height > self.bounds.size.height) {
		        CGPoint bottomOffset = CGPointMake(
		                0, self.scrollView.contentSize.height - self.bounds.size.height);
...
- (void)inputTextFieldEditingDidEnd {
	if (self.inputTextField.text.length > 0) {
		self.inputTextField.text =
		        [NSString stringWithFormat:@"%@ ", self.inputTextField.text];
...
- (BOOL)shouldInputTextDeleteBackward {
	...
	if (tagIndexes.count > 0) {
		for (NSInteger i = tagIndexes.count - 1; i >= 0; i--) {
			if ([self.delegate
			     respondsToSelector:@selector(tagsView:shouldRemoveTagAtIndex:)] &&
			    ![self.delegate tagsView:self
			      shouldRemoveTagAtIndex:tagIndexes[i].integerValue]) {
				continue;
			}
		...
		return NO;
	} else if ([self.inputTextField.text isEqualToString:@""] &&
	           self.mutableTags.count > 0) {
		NSInteger lastTagIndex = self.mutableTags.count - 1;
		if (self.selectBeforeRemoveOnDeleteBackward) {
			if ([self.delegate
			     respondsToSelector:@selector(tagsView:shouldSelectTagAtIndex:)] &&
			    ![self.delegate tagsView:self shouldSelectTagAtIndex:lastTagIndex]) {
				return NO;
			} else {
			...
			if ([self.delegate
			     respondsToSelector:@selector(tagsView:shouldRemoveTagAtIndex:)] &&
			    ![self.delegate tagsView:self shouldRemoveTagAtIndex:lastTagIndex]) {
				return NO;
			} else {
...
- (void)tagButtonTapped:(UIButton *)button {
	if (self.selectable) {
		...
		if (button.selected) {
			if ([self.delegate respondsToSelector:@selector(tagsView:
			                                                shouldDeselectTagAtIndex:)] &&
			    ![self.delegate tagsView:self shouldDeselectTagAtIndex:buttonIndex]) {
				return;
			}
			...
			if ([self.delegate
			     respondsToSelector:@selector(tagsView:shouldSelectTagAtIndex:)] &&
			    ![self.delegate tagsView:self shouldSelectTagAtIndex:buttonIndex]) {
				return;
			}
        ...
        replacementString:(NSString *)string;
{
	if (textField == self.textField) {
		...
		if ([string rangeOfCharacterFromSet:self.deliminater].location !=
		    NSNotFound) {
			[self inputTextFieldChanged];
		}
	}
